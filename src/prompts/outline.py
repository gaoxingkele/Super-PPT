...
- 每页标题必须是一个**完整的断言句**（可被验证的结论），而非主题标签
  - ✗ 错误示例："市场分析"、"技术方案"、"实验结果"
  - ✓ 正确示例："中国新能源市场规模五年翻倍达1.2万亿"、"微服务架构使系统吞吐量提升3倍"
...
## 金字塔原则（Pyramid Principle）— 必须遵守！
1. **断言式标题**：每页 title 必须是完整断言句（可验证结论），不是主题标签
...
2. **精炼文字**：每页 bullet 3~5 条，每条 15-35 字，包含具体数据
...
- data_chart: 数据展示页，配 matplotlib 图表。必须包含 bullets 字段，提供 2-3 条关键数据洞察/结论，解释图表数据的含义和启示，而非仅展示数据
